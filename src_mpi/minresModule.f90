...
!   g95 -c -g -O0 -pedantic -Wall -Wextra -fbounds-check -ftrace=full
!   minresModule.f90
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
contains
  ...
  subroutine MINRES( n, b, shift, checkA, precon, x, itnlim, nout, rtol, &
...
!     Local arrays and variables
    ...
    y      = b
    r1     = b
    if ( precon ) call precodition(istot,b,n,grid,ipatch,y)
    ...
    if (checkA  .and.  precon) then
       call precodition(istot,y,n,grid,ipatch,r2)
       s      = dot_product(y ,y )
       t      = dot_product(r1,r2)
       z      = abs(s - t)
    ...
    !-------------------------------------------------------------------
    if (checkA) then
       call blurring (istot, ipatch, grid, inv, ir, wd, wb, n, y, w )
       call blurring (istot, ipatch, grid, inv, ir, wd, wb, n, w, r2 )
       s      = dot_product(w,w )
       t      = dot_product(y,r2)
       z      = abs(s - t)
       epsa   = (s + eps) * eps**0.33333
    ...
    do
       ...
       call blurring (istot, ipatch, grid, inv, ir, wd, wb, n, v, y )
       ...
       r1     = r2
       r2     = y
       if ( precon ) call precodition(istot,r2,n,grid,ipatch,y)
       ...
       if (istop == 0) then
          ...
          if (qrnorm/beta1 <= 0.05  .or.  relArnorml <= epsr) istop = 1
       end if
...
!*******************************
subroutine blurring(istot,ipatch,grid,inv,ir,wd,wb,nn,deltade1d,Hdeltade1d)
  use parameters
  use kwsinc
  implicit none
  include 'fftw3.f'
  ...
  real, allocatable                 :: deltade(:,:),deltade2(:,:),Hdeltade(:,:)
  integer                           :: ir(grid%nrecmax,5)
  real                              :: wd(grid%nt,grid%nrecmax),wb(grid%n1,grid%n2)

  allocate(deltade(grid%nt,grid%nrec(istot)))
  allocate(Hdeltade(grid%nt,grid%nrec(istot)),deltade2(grid%nt,grid%nrec(istot)))
  ...
  do i=1, grid%nt
     deltade(grid%nt-i+1,j)=wd(i,j)*deltade(grid%nt-i+1,j)
  enddo
  enddo

  ! adjoint simulation
  call submodeling_adjoint_esource_data(istot,grid%c,grid%n1,grid%n2,grid%npml,grid%h,grid%ofs,grid%nt,grid%dt,grid%nto,grid%dto,&
              deltade,grid%type_rec,grid%nrec(istot),grid%nrecmax,ir,wb,deltade2)

  do j=1, grid%nrec(istot)
  do i=1, grid%nt
     deltade2(i,j)=wd(i,j)*deltade2(i,j)
  enddo
  enddo

  call datanorm(deltade2,grid%nt,grid%nrec(istot),deltade2_norm)
  call datanorm(deltade,grid%nt,grid%nrec(istot),deltade_norm)

  fac=deltade2_norm/deltade_norm*inv%mu
  ...
  call sub2d21d(Hdeltade,grid%nrec(istot),grid%nt,nn,Hdeltade1d)

  deallocate(deltade,Hdeltade,deltade2)

end subroutine

subroutine precodition(istot,res1d,nn,grid,ipatch,res_preco1d)
  use parameters
  implicit none
  include 'fftw3.f'
  type (grid_type)            :: grid
  integer                     :: i,j,k,l,ipatch,nto,nwt,nwr,nfo,nn,istot
  real                        :: res1d(nn), res_preco1d(nn)
  real, allocatable           :: res(:,:),res_preco(:,:)

  allocate(res(grid%nt,grid%nrec(istot)),res_preco(grid%nt,grid%nrec(istot)))

  ! down sample the data

  call sub1d22d(res1d,grid%nrec(istot),grid%nt,nn,res)

  call filmartin2d(res,grid%nrec(istot),grid%nt,grid%dt,grid%flow,grid%fhig,grid%llow,grid%lhig,res_preco)

  ! transform the data from 2D to 1D
  call sub2d21d(res_preco,grid%nrec(istot),grid%nt,nn,res_preco1d)

  deallocate(res,res_preco)
